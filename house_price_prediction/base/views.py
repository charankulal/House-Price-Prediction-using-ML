...
hybrid_model = joblib.load('D:\\PROJECTS\\House Price Prediction System\\house_price_prediction\\model\\hybrid_model.pkl')
...
def dashboard(request):
    if request.method == 'POST':
        ...
        if request.POST.get('city')!='' and request.POST.get('location')!='' and request.POST.get('area')!='' and request.POST.get('bedrooms')!='':
            # Make prediction
            data = {
            ...
            user = request.user
            prediction_instance=Prediction(user=user,city=data['City'][0],location=data['Location'][0],area=data['Area'][0],bedrooms=data['No. of Bedrooms'][0], predicted_price=predicted_price)
            prediction_instance.save()
            return render(request, 'base/results.html', {'prediction': predicted_price,'pred_form': form})
        else:
